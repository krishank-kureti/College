...
UPDATE stall_items SET total_quantity = 25 
WHERE item_name = 'Mushroom Risotto' AND stall_id = 'S1';
SELECT * FROM stall_items;
  
-- 3)
SELECT * FROM registration;
DELETE FROM registration WHERE event_id = 'E1' AND SRN LIKE 'P100_';
SELECT * FROM registration;

-- 4)
SELECT * FROM purchased;
INSERT INTO purchased 
VALUES ('P1017', 'S6', 'Fish Tacos', '2023-07-10 14:00:00', 3);
SELECT * FROM purchased;
...
-- 6)
SELECT participant.name, participant.SRN, group_concat(visitor.name), count(visitor.name)
FROM participant LEFT JOIN visitor ON participant.SRN = visitor.SRN GROUP BY participant.SRN;

-- 7)
SELECT e.event_id, e.event_name
FROM event e JOIN  registration r on e.event_id = r.event_id 
JOIN participant p ON r.SRN = p.SRN
GROUP BY e.event_id, e.event_name HAVING sum(p.gender='Male') = sum(p.gender='Female');

-- 8)
SELECT e.event_name,(f.year > 2047) as afterGJ FROM event e JOIN team t ON e.team_id = t.team_id
